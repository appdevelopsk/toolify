...
export default function GravelCalculator() {
  const t = useTranslations("tools.gravel-calculator");
  ...
  const result = useMemo(() => {
    ...
    if (![l, w, d].every(isFinite) || l <= 0 || w <= 0 || d <= 0) return null;
    const cuyd = (l * w * (d / 12)) / 27;
    return { cuyd, tons: cuyd * 1.4 };
  }, [length, width, depth]);
  ...
  return (
    <div>
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        {field(t("input.depth"), depth, setDepth, "3")}
      </div>
        ...
        {result === null ? (
          ...
          <>
            ...
            <p className="text-3xl font-bold">
              ...
              <span className="ml-2 text-base font-normal text-slate-500">≈ {fmt.format(result.tons)} t</span>
            </p>
          </>
        )}
      </div>
